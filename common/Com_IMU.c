 ...
 * @return {*}
 */
void Com_IMU_GetEulerAngle(GyroAccel_Struct  *gyroacc,
    ...
    float              dt)
{
    ...
    // 加速度归一化
    NormQuat = Q_rsqrt(squa(gyroacc->accel.x) +
                       squa(gyroacc->accel.y) +
                       squa(gyroacc->accel.z));

    Acc.x = gyroacc->accel.x * NormQuat;
    Acc.y = gyroacc->accel.y * NormQuat;
    Acc.z = gyroacc->accel.z * NormQuat;
    // 向量差乘得出的值
    ...
    normAccz = gyroacc->accel.x * vecxZ + gyroacc->accel.y * vecyZ + gyroacc->accel.z * veczZ; /*Z轴垂直方向上的加速度，此值涵盖了倾斜时在Z轴角速度的向量和，不是单纯重力感应得出的值*/
}
